...
int main()
{
  ...
  if (pipe(pfds) == -1) { perror("pipe"); return 2; }

  buffer_init(&buffer_pr, read, pfds[0], prbuf, sizeof(prbuf));
  buffer_init(&buffer_pw, write, pfds[1], pwbuf, sizeof(pwbuf));
 
  if (write(pfds[1], str, 8) == -1) { perror("write"); return 2; }
  if (read(pfds[0], buf, 8) == -1)  { perror("read"); return 2; }
